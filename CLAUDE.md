...
**Two known defects, both found by running it on a device and neither fixed:**

1. **The overlay panels render around 50% transparent and faces remain readable.** Measured: a
   covered face spans a 59-level luma range against 120 uncovered. The paint is `argb(255,…)`, the
   window is `alpha = 1f`, and `PorterDuff.Mode.SRC` changed nothing, so the alpha is being applied
   by the system to the `TYPE_APPLICATION_OVERLAY` window rather than by our drawing. Cause not yet
   identified. `FLAG_SECURE` is *not* the workaround — it blanks every screenshot and screen
   recording on the device while active (verified).
2. **Target selection looks wrong in a group photo** — men appear covered with the target set to
   women. Person boxes overlap heavily in a group shot and a box with no matched face is covered by
   design, so this may be correct-but-coarse rather than a bug; it has not been diagnosed.

Until (1) is resolved the Android feature does not deliver its promise, however healthy the pipeline
looks. Everything behind it is verified: all three models load and run in the minified release
build, `app/src/androidTest/.../BlurPipelineTest.kt` passes 16 tests on-device, and panels are
placed on the right people.
