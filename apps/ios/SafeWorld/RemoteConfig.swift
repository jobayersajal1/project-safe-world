...
/// only changes via an app update.
///
...
enum RemoteConfig {
    ...
    /// GitHub Pages. iOS fetches the **scrambled** variant: Safari's content
    /// blocker matches from a rule file containing literal domains, so the app
    /// has to recover the real names at runtime and can't use Android's
    /// one-way digests.
    ///
